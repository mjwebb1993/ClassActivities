...
data.forEach(anyWord => console.log(anyWord.address.city));
// create an array of latitude and longitude coordinate pairs
const coords = [];
data.forEach(location => {
  let lat = location.address.geo.lat;
  let lng = location.address.geo.lng;
  coords.push([lat, lng]);
});
// console.log(coords);
...
data.forEach(item => {
  let catchString = `${item.company.catchPhrase} SPLIT ${item.company.bs}`;
  arrOfString.push(catchString);
});
...
classes.forEach(
  (htmlElement, i) =>
    (htmlElement.textContent = `text content is change, class week number ${i}`)
);
